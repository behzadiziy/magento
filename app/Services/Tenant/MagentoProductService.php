...
class MagentoProductService
{
    ...
    public function getProductsForStore(int $websiteId): array
    {
        ...
        $endpoint = "{$this->apiUrl}/rest/V1/products?" . http_build_query($searchCriteria);
    ...
    {
        $token = $this->getAccessToken();
        $endpoint = "{$this->apiUrl}/rest/V1/products/attributes/{$attributeCode}";
    ...
    {
        $token = $this->getAccessToken();
        $endpoint = "{$this->apiUrl}/rest/V1/products/attribute-sets/groups";
    ...
    public function assignAttributeToAttributeSet(string $attributeCode, int $attributeSetId): bool
    {
        ...
        $assignEndpoint = "{$this->apiUrl}/rest/V1/products/attribute-sets/attributes";

        try {
            $response = $this->client->post($assignEndpoint, [
                'headers' => [
                    'Authorization' => 'Bearer ' . $token,
                    'Content-Type' => 'application/json',
                ],
    ...
    {
        $token = $this->getAccessToken();
        $endpoint = "{$this->apiUrl}/rest/V1/products/attributes";
    ...
    public function storeProductForStore(array $product, int $websiteId , $images): array
    {
        ...
        if ($this->checkProductExist($product['sku'], $websiteId)) {
            throw new \Exception(
        ...
        $token = $this->getAccessToken();
        $endpoint = "{$this->apiUrl}/rest/V1/products";
        ...
        try {
            ...
            if (!empty($images)) {
                try {
                    $this->addMediaToProduct($createdProduct['sku'], $images);
    ...
    public function addMediaToProduct(string $sku, array $images): array
    {
        ...
        $token = $this->getAccessToken();
        $endpoint = "{$this->apiUrl}/rest/V1/products/" . urlencode($sku) . "/media";
        $mediaEntryIds = [];
    ...
    {
        $token = $this->getAccessToken();
        $endpoint = "{$this->apiUrl}/rest/V1/products/" . urlencode($sku);
